...
void vector_swap(vector<int> &v1,vector<int> &v2,
    ...
    int start2, int end2) {
       ...
       int i = 0;
       for(e : temp1){
         v2.insert((v2.begin()+start2)+i,e);
         i++;
       }

       i = 0;
       for(e : temp2){
         v1.insert((v1.begin()+start1)+i,e);
         i++;
       }

       v1.erase(v1.begin()+start1+end2-start2, v1.begin()+start1+end2-start2+end1-start1);
       v2.erase(v2.begin()+start2+end1-start1, v2.begin()+start2+end1-start1+end2-start2);
}
int main() {
  //read input
  int n,c;
